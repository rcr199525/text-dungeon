...
from dictionaries.items import *
from dictionaries.monsters import *
...
class Dungeon(cmd.Cmd):
    ...
    # 2. Multiple versions of same context to add variety to the dialogue
    # main loop
    INV_INTRO = 'Pockets'
...
Check these, perhaps? NORTH/SOUTH/EAST/WEST or UP/DOWN'''
    ...
    PICK_ITEM = ['You stuff that', 'in your pocket']
    # (pick)
    ...
    NOT_EDIBLE = [
    ['This', 'is too hard for me to eat'],
    ...
    SWALLOW_SYNONYMS = ['consume', 'devour', 'gulp down', 'eat', 'swallow']
    ...
    NOT_SOLD_HERE = "That item isn't for sale"
   ...
   # combat
    ...
    def __init__(self, player, rooms):
        ...
        self.coin_hack(50)
        self.rooms = rooms
        self.intro = input(center_screen(banner('<Welcome to text dungeon. Press Enter to Continue>')))
    ...
    def check_coins(self):
        '''Converts any 'money' item in inventory to the user's 'coin' counter based on the COIN_VALUE dictionary'''
    ...
    def coin_hack(self, amount):
        '''Easy coin hack, just specify the amount'''
        coins = ['coin'] * amount
    ...
    def display_current_room(self):
        ...
        if current_room[SHOP]:
            ...
            for item_name in current_room[SHOP]:
                ...
                print('{}{} {}{}({}$)'.format(BULLET, item[NAME],
                '-' * (ln - len(item[NAME])), ' ' * (3 - lnp), item[PRICE]))
                self.reset_color()
            print()
        ...
        for k, v in get_room_exits(current_room).items():
            print('{}{}| {}'.format(k.upper(), (5 - len(k)) * ' ', v))
        print()
        self.reset_color()
    ...
    def display_player_info(self):
        ...
        p.skill_type[NAME],
        self.coins)
        print(' \{}| {} {}'.format('_' * EXTENSION, c_user_stats, '|__/'))
        # Print bot border
        print(' ' * (EXTENSION + 2) + '└' + '-' * x + '┘')
    ...
    def ask_fight_or_flight(self, target_room, dir):
        '''prompts user to accept fight, returns True if accepted False if retreat chosen'''

        clear()
        print(center_screen("Will you face them with courage? or . . run away?"), end='')
        sleep(2)
        ...
        while True:
            ...
            if answer.lower() == 'y':
                ...
                clear()
                print('', end=''); headline('# Prepare for fight!')
                print('', end='')
                print(get_tag_items(self.inventory, 'weapon'))
                ...
                if my_weapons:
                    ...
                    for key, value in my_weapons.items():
    ...
    def sort_inventory_items(self, item_names):
        '''Sorts items in a list of items (particularly self.inventory) according to tags and prints them'''
    ...
    def go_new_location(self, inp):
        ...
        for dir in DIRECTIONS:
            ...
            if current_room[dir] and dir == inp:
                ...
                if inp == UP: load_text='Going up'
                elif inp == DOWN: load_text='Going down'
        ...
        def _do_pick_single_item(item_name: str) -> None:
            ...
            if item_name in current_room[GROUND]:
                ...
                if item[PICKABLE]:
                    ...
                    item[NAME].lower(), self.NOT_PICKABLE[1]))
            # Empty input
            elif not item_name:
                self.display_current_room()
    ...
    def do_eat(self, arg):
        ...
        if arg.lower() in self.inventory:
            if ITEMS[arg.lower()][EDIBLE] == True:
                ...
                self.display_current_room()
                self.achieve_msg(s[0] + ' {} '.format(arg.lower()) + s[1])
        # Empty input
        elif not arg:
            self.display_current_room()
        ...
        # If item isn't in inventory check if such item exists and prompt accordingly
            ...
            if arg.lower() in ITEMS:
                self.display_current_room()
                x = '{} {} {}'.format(use_an(arg.lower(), True), arg.lower(), self.BAD_FOOD)
                self.error_msg(x)
            else:
                self.display_current_room()
                self.error_msg('"{}"{}'.format(arg, self.UNKNOWN_ITEM))
    ...
    def do_drop(self, arg):
        ...
        if arg.lower() in self.inventory:
            ...
            x = 'You dropped ' + use_an(arg.lower()) + ' ' + arg.lower()
            # Remove item from inventory then display room
    ...
    def do_buy(self, arg):
        '''Buys an item from a shop (Adds it to inventory, removes it from current_room's SHOP)'''
        current_room = ROOMS[self.location]
...
if __name__ == '__main__':
    me = player.Player('Steve', 10, WEAPONS[FIST])
